...
const AllJobsPage = () => {
  ...
  const debouncedQueries = {
    tags: useDebounce(queries.tags, 500),
    locations: useDebounce(queries.locations, 500),
    companies: useDebounce(queries.companies, 500),
    vias: useDebounce(queries.vias, 500),
  };
  ...
  return (
    ...
    >
      ...
      <motion.div className="relative z-10 flex flex-col lg:flex-row gap-6 px-4 sm:px-6 md:px-8 py-6">
        <motion.div className="w-full lg:max-w-xs space-y-4" variants={sidebarVariants} initial="initial" animate="animate">
          <motion.div className="bg-white/70 backdrop-blur-lg rounded-2xl p-6 shadow-lg border border-white/20" variants={filterItemVariants}>
            <h3 className="text-lg font-semibold mb-4 text-gray-800">Filters</h3>
            <div className="space-y-4">
              {filterTypes.map(({ key, label }, index) => (
                <motion.div key={key} variants={filterItemVariants} custom={index}>
                  <FilterInput
                    label={label}
                    query={queries[key]}
        ...
        <motion.div className="flex-1" variants={contentVariants} initial="initial" animate="animate">
          <motion.div className="bg-white/60 backdrop-blur-lg rounded-2xl p-6 shadow-lg border border-white/20 mb-6" variants={filterItemVariants}>
            <h2 className="text-xl sm:text-2xl font-bold text-gray-800 mb-2">Available Positions ({jobs.length})</h2>
            <p className="text-gray-600">Discover your next career opportunity</p>
          </motion.div>

          <motion.div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6" variants={contentVariants}>
            <AnimatePresence>
              {jobs.map((job, index) => (
                <motion.div
                  key={job.id}
                  className="bg-white/70 backdrop-blur-lg border border-white/20 rounded-2xl shadow-lg p-4 sm:p-6 flex flex-col hover:shadow-xl transition-all duration-300"
                  variants={jobCardVariants}
                  custom={index}
                  whileHover={{ scale: 1.02, y: -5, boxShadow: "0 20px 40px rgba(0,0,0,0.1)" }}
                  layout
                >
                  <motion.div className="flex items-center mb-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }}>
                    {job.companyLogo ? (
                      <img src={job.companyLogo} alt={`${job.companyName} logo`} className="w-16 h-16 object-contain rounded-xl" />
                    ) : (
                    ...
                    <div className="ml-4">
                      ...
                      <p className="text-sm text-gray-500">{job.location} • {job.experience} • {job.jobType}</p>
                    </div>
                  </motion.div>

                  <p className="text-xs sm:text-sm text-gray-700 line-clamp-3 mb-4 flex-grow">{job.description}</p>
                  ...
                  <div className="text-xs text-gray-500 mb-4 flex justify-between">
                    ...
                    <div className="flex flex-col">
                      {new Date(job.lastVerified).toLocaleDateString() !== "1/1/1970" && (
                        <span>Last verified: {new Date(job.lastVerified).toLocaleDateString()}</span>
                      )}
          ...
          <motion.div className="flex flex-wrap items-center justify-center gap-4 mt-8 px-4" variants={filterItemVariants}>
            <motion.button
              disabled={page <= 1}
